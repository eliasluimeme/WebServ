...
std::string response::exec_outfile(std::string inFile, std::map<std::string, std::string> reqHeader)
{
	...
	std::cout << "Program name " << const_cast<char *>((*reqHeader.find("Program_Name:")).second.c_str()) << "file name " << const_cast<char *>((*reqHeader.find("File_Name:")).second.c_str()) << std::endl;
	pid_t pid = fork();
	if (pid == -1)
		return "";
	if (pid == 0)
